...
CREATE TABLE IF NOT EXISTS users (
    ...
    email       VARCHAR(40) UNIQUE NOT NULL, -- 이메일 (필요할까?)
...
CREATE TABLE IF NOT EXISTS chat_rooms (
    id          SERIAL PRIMARY KEY,
    member_id   INTEGER, -- 회원 아아디 FK
    title       VARCHAR(20), -- 대화방 제목
    persona     VARCHAR(50), -- 페르소나 종류
    created_at  TIMESTAMP DEFAULT NOW(), -- 생성일
...
COMMENT ON COLUMN chat_rooms.member_id IS '회원 아이디 FK';
...
CREATE TABLE IF NOT EXISTS messages (
    id          SERIAL PRIMARY KEY,
    room_id     INTEGER, -- 대화방 FK
    role        message_role, -- 'user' or 'assistant'
    content     TEXT, -- 내용
